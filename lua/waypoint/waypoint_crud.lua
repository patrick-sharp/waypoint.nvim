...
function M.insert_waypoint(filepath, line_nr, annotation)
  ...
  elseif state.wpi == #state.waypoints then
    ...
    change_wpi = #state.waypoints - 1
  else
    ---@type waypoint.Waypoint[]
    local new_waypoints = {}
    for i = 1,state.wpi-1 do
...
function M.append_waypoint(filepath, line_nr, annotation)
  ...
  elseif state.wpi == #state.waypoints then
    ...
    new_waypoints[state.wpi+1] = waypoint
...
function M.append_waypoint_end_wrapper()
  if not u.is_file_buffer() then return end
  local filepath = vim.fn.expand("%")
  local cur_line_nr = vim.api.nvim_win_get_cursor(0)[1] -- Get current line number (one-indexed)
  M.append_waypoint_end(filepath, cur_line_nr, nil)
end

function M.insert_waypoint_beginning_wrapper()
  if not u.is_file_buffer() then return end
  local filepath = vim.fn.expand("%")
  local cur_line_nr = vim.api.nvim_win_get_cursor(0)[1] -- Get current line number (one-indexed)
  M.insert_waypoint_beginning(filepath, cur_line_nr, nil)
end

---@param waypoint_idx integer
function M.edit_waypoint_name(waypoint_idx)
  ---@type waypoint.Waypoint[]
  local waypoints
...
function M.edit_waypoint_name_wrapper()
  if not u.is_file_buffer() then return end
  ...
  M.edit_waypoint_name(existing_waypoint_i)
end

---@param waypoint_idx integer
function M.clear_waypoint_name(waypoint_idx)
  ---@type waypoint.Waypoint[]
  local waypoints
  if state.sort_by_file_and_line then
    waypoints = state.sorted_waypoints
...
function M.delete_selected_waypoints()
